...
async function checkout(req, res) {
  const { customer, items, total, shippingFee, couponCode, couponId } = req.body
  ...
  // The frontend already calculates the discounted total (cartSubtotal - discount + shipping)
  // We store this as-is and just record the discount amount for reference.
  let finalTotal = Number(total);
  let discountAmount = 0;
  let appliedCouponCode = null;

  // Calculate the original subtotal (before discount) from cart items for email/reference
  const subtotal = items.reduce((sum, item) => sum + (Number(item.price) || 0) * (item.quantity || 1), 0);

  // If a coupon was provided, validate it and record the discount
  if (couponCode && couponId) {
    const couponResult = await applyCouponToOrder(couponId, subtotal, items);
    if (couponResult) {
      discountAmount = couponResult.discount;
      appliedCouponCode = couponResult.code;
      // Do NOT modify finalTotal - frontend already calculated it correctly
    } else {
  ...
  try {
    const orderRes = await pool.query(
      `INSERT INTO orders
        (customer_name, customer_email, customer_phone, customer_address,
         total_amount, subtotal, order_items, payment_method, status, shipping_fee, province, city_id,
         coupon_code, discount_amount)
       VALUES ($1,$2,$3,$4,$5,$6,$7,'cod','pending_confirmation',$8,$9,$10,$11,$12)
       RETURNING id`,
      [
        customer.name,
        customer.email,
    ...
    // Send notification email to admin with full pricing breakdown
    try {
      await sendOrderNotificationEmail(
        orderId, customer, items, finalTotal, 'cod', 'pending_confirmation',
        shippingFee, customer.province, customer.city,
        subtotal, discountAmount, appliedCouponCode
      )
    } catch (emailError) {
...
async function confirmOrderWithTrax(req, res) {
  ...
  try {
    ...
    const traxPayload = {
      ...
      item_product_type_id: Number(process.env.TRAX_PRODUCT_TYPE_ID || 24),
